import React from 'react';
import { useAuth } from '../context/AuthContext'; // Adjust path if necessary
import { Navigate } from 'react-router-dom';

const Login = () => {
    const { currentUser, loginWithGoogle } = useAuth();

    const handleGoogleLogin = async () => {
        try {
            await loginWithGoogle();
            // Successful login will be handled by onAuthStateChanged
            // and currentUser will be updated, triggering a re-render or redirect.
        } catch (error) {
            console.error("Login failed:", error);
            // You might want to display an error message to the user here
        }
    };

    // If user is already logged in, redirect them from the login page
    // For example, redirect to the homepage or a dashboard.
    // This assumes you have a route for '/' (homepage).
    if (currentUser) {
        return <Navigate to="/" replace />;
    }

    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
            <div className="p-8 bg-white rounded-lg shadow-md w-full max-w-md">
                <h1 className="text-2xl font-bold text-center text-mentone-navy mb-6">
                    Login to Hockey Tracker
                </h1>
                <button
                    onClick={handleGoogleLogin}
                    className="w-full flex items-center justify-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-mentone-skyblue hover:bg-mentone-skyblue/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-mentone-skyblue"
                >
                    <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" d="M10 0C4.477 0 0 4.477 0 10c0 4.418 2.865 8.148 6.839 9.489.5.092.682-.217.682-.483 0-.237-.009-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0110 4.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.026 2.747-1.026.546 1.378.201 2.397.098 2.65.64.7 1.028 1.595 1.028 2.688 0 3.848-2.338 4.695-4.566 4.943.359.308.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.001 10.001 0 0020 10c0-5.523-4.477-10-10-10z" clipRule="evenodd" />
                    </svg>
                     Sign in with Google
                </button>
                {/* Placeholder for Email/Password form */}
                {/* 
                <div className="mt-6">
                    <p className="text-center text-sm text-gray-600">Or sign in with email</p>
                    // Add email/password form here if needed
                </div>
                */}
            </div>
        </div>
    );
};

export default Login;
