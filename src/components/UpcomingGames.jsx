...
const UpcomingGames = () => {
    ...
    const fetchUpcomingGames = async (filter) => {
        try {
            ...
            // No need for client-side filtering for Mentone games anymore
            // const mentoneGames = gamesData.filter(...) // <-- REMOVED
    ...
    const getCompetitionName = (game) => {
        ...
        return `Unknown Competition (Fixture: ${fixtureId || 'N/A'})`;
    };
    ...
    const generateHTMLTable = () => {
        ...
        Object.entries(grouped).forEach(([date, dateGames]) => {
            ...
            dateGames.forEach((game, index) => {
                ...
                // Checks still needed for display/styling
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                            ...
                            {Object.entries(groupedGames).map(([date, dateGames]) => (
                                <div key={date} className="relative">
                                    ...
                                    <div className="space-y-4">
                                        {dateGames.map((game) => {
                                            // Checks still needed for display
                                            const isMentoneHome = game.home_team?.club?.toLowerCase() === "mentone";
                                            const isMentoneAway = game.away_team?.club?.toLowerCase() === "mentone";
                                            ...
                                            return (
                                                <div key={game.id} className="bg-white border border-gray-100 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300 transform hover:translate-y-[-2px]" >
                                                    ...
                                                    <div className="bg-gray-50 px-4 py-2 border-b border-gray-100 flex justify-between items-center"> <div className="flex items-center"> <span className="font-semibold text-mentone-navy"> {competitionName} </span> </div> <div className="flex items-center space-x-2"> {game.round && ( <span className="text-xs px-2 py-0.5 bg-mentone-gold/80 text-mentone-navy rounded-full font-medium"> Round {game.round} </span> )} </div> </div>
                                                    {/* Card Body: Teams & Info */}
                                                    <div className="p-4">
                                                        {/* Teams Row */}
                                                        <div className="flex items-center justify-between mb-5"> <div className="flex flex-col items-center w-5/12"> <span className={`text-center font-medium ${isMentoneHome ? "text-mentone-skyblue" : "text-gray-700"}`}> {isMentoneHome ? "Mentone" : game.home_team?.name || "TBD"} </span> </div> <div className="flex flex-col items-center justify-center"> <span className="text-gray-400 text-sm font-medium">vs</span> </div> <div className="flex flex-col items-center w-5/12"> <span className={`text-center font-medium ${isMentoneAway ? "text-mentone-skyblue" : "text-gray-700"}`}> {isMentoneAway ? "Mentone" : game.away_team?.name || "TBD"} </span> </div> </div>
                                                        {/* Info Row: Time & Venue */}
                                                        <div className="flex justify-between items-center pt-3 border-t border-gray-100"> <div className="flex items-center text-gray-600"> <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /> </svg> <span className="text-sm">{formatGameTime(game.date)}</span> </div> <div className="flex items-center text-gray-600 min-w-0"> {/* Added min-w-0 for truncation */} <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /> </svg> <span className="text-sm truncate">{game.venue || "Venue TBD"}</span> {/* Added truncate */} </div> </div>
                                                    </div>
                                                </div>
                                            );
                                        })}
            ...
            {viewMode === "table" && (
                <div className="p-5">
                    {filteredGames.length === 0 ? (
                        ...
                        <div>
                            {/* Copy button */}
                            <div className="flex justify-end mb-4"> <button onClick={copyHTMLToClipboard} className={`px-4 py-2 rounded-lg text-white text-sm flex items-center ${textCopied ? 'bg-green-600' : 'bg-mentone-skyblue hover:bg-mentone-skyblue/90'} transition-colors`} > {textCopied ? ( <><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Copied!</> ) : ( <><svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" /></svg>Copy HTML</> )} </button> </div>
                            {/* Table Display */}
                            <div ref={tableContainerRef} className="overflow-auto bg-white rounded-lg" >
                                {Object.entries(groupedGames).map(([date, dateGames]) => (
                                    <div key={date} className="mb-8">
                                        <h3 className="text-lg font-bold text-mentone-navy mb-3 pb-2 border-b border-gray-200"> {date} </h3>
                                        <div className="overflow-x-auto">
                                            <table className="min-w-full divide-y divide-gray-200">
                                                ...
                                                <tr>
                                                    ...
                                                    <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Playing</th> {/* Changed Header */}
                                                ...
                                                {dateGames.map((game, index) => {
                                                    ...
                                                    return (
                                                        <tr key={game.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500"> {formatGameTime(game.date)} </td>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-left"> {competitionName} </td>
                                                            {/* Show Opponent Name */}
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-center font-medium text-gray-900"> {opponentName} </td>
                                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500"> {(game.venue || "Venue TBD").trim()} </td>
                                                        </tr>
                                                    );
                                                })}
                                                </tbody>
            ...
            {process.env.NODE_ENV === 'development' && (
                <div className="mt-4 p-2 border-t text-xs text-gray-500">
                    ...
                    {/* You could add more debug info here if needed */}
                </div>
            )}
